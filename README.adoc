...
précédemment crée (second argument). Le résultat dégagé suite à l'appel de `$this->assertEquals` est géré par `PHPUnit`
...
WARNING: Attention : la méthode toString fait partie des méthodes dites _magique_ en PHP. À ce sujet vous consulterez
